...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 24 19:51:57 2023
...
log_file_dir = '/Users/welcome870117/Desktop/git_project/Quantitative_trading_strategy/trading_system_v3/long_short_strategy_log.csv'
    ...
    for data in price_list:
        ...
        try:
            ...
            if quantity != False:
                ...
                new_row = {'timestamp': current_time,'symbol':data[0], 'side':'BUY', 'quantity':quantity, 'K_value':K_VALUE}
    ...
    for i in range(len(price_list)-1, 0, -1):
        ...
        try:
            ...
            if quantity != False:
                    ...
                    current_time = pd.to_datetime(datetime.now())
                    new_row = {'timestamp':current_time, 'symbol':price_list[i][0], 'side':'SELL', 'quantity':quantity, 'K_value':K_VALUE}
                    df = df.append(new_row, ignore_index=True)
                    print('----------')
        except Exception as e:
                print("error", str(e))
...
def add_trade_log(client, df, csv_dir, long_position, short_position):
    ...
    for symbol in long_position:
        ...
        # get position info
        position_info = client.future_check_position(symbol)
...
def close_position_conditional_judgment(client, csv_dir):
    ...
    for i in range(len(log_file)):
        ...
        if timestamp + k_value < current_time:
            ...
            # close position
            print('close')
            quantity = client.future_perpetual_partial_close_position(log_file['symbol'][i], log_file['quantity'][i])
            if quantity == False:
